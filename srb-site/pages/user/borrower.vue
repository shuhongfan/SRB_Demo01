...
export default {
  ...
  methods: {
    async save() {
      ...
      var res = await this.$axios.$post(`/api/core/borrower/auth/save`,this.borrower);
      if (res.code === 0) {
        this.active = 1;
      }
    },
    ...
    onUploadSuccess(response, file, type) {
      if (response.code !== 0) { // 业务失败
        this.$message.error(response.message)
        return
      }
      // 业务成功，填充上传文件borrowerAttachList列表
    ...
    // 上传失败回调，删除borrowerAttachList列表
    async onUploadRemove(file, fileList) {
      console.log('file', file)
      console.log('fileList', fileList)
      //删除oss服务器上的内容
      let response = await this.$axios.$delete('/api/oss/file/remove?url=' + file.response.data.url);
      if (response.code === 0) {
        console.log('远程删除文件成功！！！')
        // filter方法 返回值为true保留，false去除
    ...
    async initSelected() {
      // 学历列表
      let education = await this.$axios.$get(`/api/core/dict/findByDictCode/education`);
      if (education.code === 0) {
        this.educationList = education.data.dictList;
      }

      //行业列表
      let industry = await this.$axios.$get(`/api/core/dict/findByDictCode/industry`);
      if (industry.code === 0) {
        this.industryList = industry.data.dictList;
      }

      //收入列表
      let income = await this.$axios.$get(`/api/core/dict/findByDictCode/income`);
      if (income.code === 0) {
        this.incomeList = income.data.dictList;
      }

      //还款来源列表
      let returnSource = await this.$axios.$get(`/api/core/dict/findByDictCode/returnSource`);
      if (returnSource.code === 0) {
        this.returnSourceList = returnSource.data.dictList;
      }

      //联系人关系列表
      let relation = await this.$axios.$get(`/api/core/dict/findByDictCode/relation`);
      if (relation.code === 0) {
        this.contactsRelationList = relation.data.dictList;
      }
    },
    /**
     * 获取借款人认证状态
     ...
     */
    async getUserInfo() {
      let res = await this.$axios.$get(`/api/core/borrower/auth/getBorrowerStatus`);
      if (res.code === 0) {
        this.borrowerStatus = res.data.borrowerStatus;

        if (this.borrowerStatus === 0) {//未认证
          this.active = 0
          //获取下拉列表
          this.initSelected()
        } else if (this.borrowerStatus === 1) {//认证中
          this.active = 1
        } else if (this.borrowerStatus === 2) {//认证成功
          this.active = 2
        } else if (this.borrowerStatus === -1) {//认证失败
          this.active = 2
        }
      }
    },
