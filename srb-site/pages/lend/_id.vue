...
export default {
  ...
  methods: {
    ...
    async fetchAccount() {
      ...
      if (userInfo) {
        let res = await this.$axios.$get(`/api/core/userAccount/auth/getAccount`);
        if (res.code === 0) {
    ...
    async getInterestCount() {
      let res = await this.$axios.$get(`/api/core/lend/getInterestCount/
    ...
    commitInvest() {
      ...
      // 判断标的是否超卖：标的已投资金额+本次投资金额》标的总金额
      if (this.lend.investAmount + Number(this.invest.investAmount) > this.lend.amount) {
