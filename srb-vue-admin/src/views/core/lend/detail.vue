...
import lendItem from "@/api/core/lendItem";
import lendReturn from "@/api/core/lendReturn";
...
export default {
  ...
  methods: {
    ...
    // 投资记录
    async fetchLendItemList() {
      let res = await lendItem.list(this.$route.params.id);
      if (res.code === 0) {
        this.lendItemList = res.data.list
      }
    },
    // 还款计划
    async fetchLendReturnList() {
      let res = await lendReturn.list(this.$route.params.id);
      if (res.code === 0) {
